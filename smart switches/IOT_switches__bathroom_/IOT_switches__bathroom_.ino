...
unsigned long debounceDelay1 = 250;  
unsigned long lastDebounceTime2 = 0;
unsigned long debounceDelay2 = 250;  
unsigned long lastDebounceTime3 = 0;
unsigned long debounceDelay3 = 250;
...
void notification() {
  if (notificationstatus==1){
    if(switch1status==1&&l==0){
      Blynk.notify("Trimmer switch is ON! ");
      l=1;
    }
    else if(switch1status==0){
      l=0;
    }

    if(switch2status==1&&m==0){
      Blynk.notify("Tubeligt switch is ON!");
      m=1;
    }
    else if(switch2status==0){
      m=0;
    }

    if(switch3status==1&&n==0){
      Blynk.notify("Exhaustfan switch is ON! ");
      n=1;
    }
    else if(switch3status==0){
      n=0;
    }
  }
...
void relay(){
  ...
  digitalWrite(relay1, switch1status);
  digitalWrite(relay2, switch2status);  
  digitalWrite(relay3, switch3status);    
